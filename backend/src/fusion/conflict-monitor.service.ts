...
export class ConflictMonitorService {
  ...
  private updateMetrics(conflict: ConflictEvent): void {
    ...
    conflict.sources.forEach(source => {
      this.metrics.conflictsBySource[source] = 
        (this.metrics.conflictsBySource[source] || 0) + 1;
    });
    ...
    if (conflict.field === 'speed') {
      ...
      const totalSpeedConflicts = Object.entries(this.metrics.conflictsByField)
        .reduce((sum, [field, count]) => field === 'speed' ? sum + count : sum, 0);
      
      if (totalSpeedConflicts === 1) {
        this.metrics.averageSpeedDiff = conflict.maxDiff;
      } else {
        this.metrics.averageSpeedDiff = 
          ((this.metrics.averageSpeedDiff * (totalSpeedConflicts - 1)) + conflict.maxDiff) / totalSpeedConflicts;
      }
    }
  }
  ...
  private checkAlertConditions(conflict: ConflictEvent): void {
    ...
    if (conflict.mmsi) {
      const recentVesselConflicts = this.recentConflicts
        ...
        .filter(c => 
          (conflict.detectedAt.getTime() - c.detectedAt.getTime()) < 5 * 60 * 1000 // 5 minutes
        );
...
${this.getRecentConflicts(10).map(c => 
  `  ${c.detectedAt.toISOString()} - ${c.field}: ${c.percentageDiff.toFixed(1)}%`
).join('\n')}
    `.trim();
  }
}