...
export default function RouteMapSmall({ aircraftId, height = '400px' }: RouteMapSmallProps) {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const data = await api.get(`/aircrafts/${aircraftId}/history?page=1&pageSize=100`);
    ...
    for (let i = 1; i < positions.length - 1; i++) {
      ...
      feature.setStyle(
        new Style({
          image: new CircleStyle({
            radius: 3,
            fill: new Fill({ color: '#6ee7b7' }),
            stroke: new Stroke({ color: '#059669', width: 1 }),
          }),
        })
      );
        ...
        {selectedPosition && (
          <div className="space-y-1">
            ...
            <div className="grid grid-cols-2 gap-1">
              ...
              {selectedPosition.altitude !== null && selectedPosition.altitude !== undefined && (
                <>
                  <span className="text-gray-600">Độ cao:</span>
                  <span className="text-gray-900">{selectedPosition.altitude.toFixed(0)} ft</span>
                </>
              )}
              ...
              {selectedPosition.heading !== null && selectedPosition.heading !== undefined && (
                <>
                  <span className="text-gray-600">Hướng:</span>
                  <span className="text-gray-900">{selectedPosition.heading}°</span>
                </>
              )}
            </div>
          </div>
