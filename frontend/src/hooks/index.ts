export { useDataLoader } from './useDataLoader';
// Migrated to new refactored architecture with plugin system
export { useMapInitialization } from './useMapInitialization';
export { useMapClickHandler } from './useMapClickHandler';
export { useDrawingMode } from './useDrawingMode';
export { useRegionsRendering } from './useRegionsRendering';
export { useWebSocketHandler } from './useWebSocketHandler';
export { useFeatureUpdater } from './useFeatureUpdater';
// Deprecated: gộp cả aircraft + vessels (đã tách)
// export { useViewportDataLoader } from './useViewportDataLoader';
export { useAircraftViewportLoader } from './useAircraftViewportLoader';
export { useVesselViewportLoader } from './useVesselViewportLoader';
