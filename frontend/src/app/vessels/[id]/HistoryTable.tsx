...
export default function HistoryTable({ vesselId }: { vesselId: number }) {
  ...
  const [from, setFrom] = useState<string>(() => new Date(Date.now() - 24 * 3600 * 1000).toISOString());
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const data = await api.get(`/vessels/${vesselId}/history?${params.toString()}`);
        const positions = Array.isArray(data.positions) ? data.positions : [];
        setRows(positions);
        // Backend chưa trả total, tạm thời tính tổng xấp xỉ theo số trang đã xem
        setTotal(total == null ? positions.length : Math.max(total, (page + 1) * pageSize));
      } catch (e) {
        // ignore
      } finally {
        setLoading(false);
    ...
    <div className="mt-4">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Vĩ độ</th>
              <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Kinh độ</th>
              <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Tốc độ</th>
              <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Hướng</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200 text-sm">
            {rows.map((r) => (
              <tr key={r.id}>
                <td className="px-3 py-2 whitespace-nowrap">{new Date(r.timestamp).toLocaleString('vi-VN')}</td>
            ...
            {rows.length === 0 && !loading && (
              <tr>
                <td className="px-3 py-6 text-center text-gray-500" colSpan={6}>Không có dữ liệu</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="flex items-center justify-between mt-3">
        <div className="text-sm text-gray-600">
          Trang {page + 1}{total != null ? '' : ''}
        </div>
        <div className="space-x-2">
          <button disabled={page===0||loading} onClick={()=>setPage(Math.max(0,page-1))}
            className="px-3 py-1 border rounded disabled:opacity-50">Trước</button>
          <button disabled={rows.length < pageSize || loading} onClick={()=>setPage(page+1)}
          ...
          <select value={pageSize} onChange={(e)=>{setPage(0);setPageSize(Number(e.target.value));}}
            ...
            {[25,50,100,200].map(s=> <option key={s} value={s}>{s}/trang</option>)}
          </select>
        </div>
      </div>
    </div>
